...
## Abrirl el numfile.central ------------------------------------------------------
### para que los BBH solo sean de las semillas de la familia $numFile
#verificar la llave de hasmetCENTRAL este en numfile.central, si no, no agregar al QC

my @FAMILYSEEDS;
open (CENTRALFILE, "$OUTPUT_PATH/blast/$numFile.central")or die $!;
			# Leer la primera línea del archivo
		foreach my $linea ( <CENTRALFILE>){
			#Imprimir la primera línea
			#print MESSAGES ("linea $linea \n");
			if($linea =~/>/){
				chomp ($linea);
				$linea=~s/\r//;
				print MESSAGES ("linea  ide id $linea\n");
				$linea=~s/\|/_/g;
				$linea=~s/>//g;
				push(@FAMILYSEEDS,$linea);
				my @spltcentralID=split(/\|/,$linea);
				#print MESSAGES ("SPLTID $spltcentralID[0] $spltcentralID[1] $spltcentralID[2] \n\n");
			}
			# Cerrar el archivo
		 }
close(CENTRALFILE);

#foreach my $seed (@FAMILYSEEDS){print MESSAGES ("seed $seed\n");	}
#-----------------------------------------------------------------------------------------------------
open (MESSAGES, '>', "$OUTPUT_PATH/blast/seqf/tree/$numFile.messages.txt")or die $!;


#-----------------------------------------------------------------------------------------------------
my @QC2; #Este arreglo es para identificar los BBH con otras familias que aparecen en el arbol
...
while (<LABELS>){
   ...
   #my  $ALL_ids='';
    print MESSAGES ("LINEA $_ \n");
    print MESSAGES ("splitkey $giA[0] $giA[1] $giA[2] \n");
    # print MESSAGES ("hashkeycentral $hashMETCENTRAL{$giA[1]} \n");
    ########################### ANTISMASH ##############
    ...
    if(exists $hashNP{$giA[0]}){
	    ...
	    ########################################
           ...
           if(exists $hashMETCENTRAL{$giA[1]} and !exists $hashANTISMASHid{$giA[1]}){
		print MESSAGES ("hashMETcentral1 $hashMETCENTRAL{$giA[1]} \n");
			my @stCENTRAL=split('\t',$hashMETCENTRAL{$giA[1]});
			my $SeedMetID=$stCENTRAL[0];
			$SeedMetID=~s/\|/_/g;
		
			foreach my $seed (@FAMILYSEEDS){
			$seed=~s/CENTRAL//;
			print MESSAGES ("seed $seed  Seed Met ID $SeedMetID\n");
			if($seed eq $SeedMetID){
				print MESSAGES ("LOGRADO\n");
            			$METcentral=$giA[1].'|'.$giA[$#giA];
			   	print OUTLABELS "$_	$METcentral\n";
				print MESSAGES ("$METcentral \n ---------------------------------------\n\n");
		             	push(@QC, $METcentral);
			}
			else{
				print MESSAGES ("OTROBBH\n");
				$METcentral=$giA[1].'|'.$giA[$#giA];
				print OUTLABELS "$_     $METcentral\n";
				#$PINKBBHcircle=$PINKBBHcircle. qq|"<circle style='fill:pink;stroke:black' r='9'/>" I $keyCENT\n|; ## Circle for seed central pÃ¡thway
				#$PINKBBHstring=$PINKBBHstring.'"stroke-width:7; stroke:pink" Individual '.$keyCENT."\n";
				push(@QC2, $METcentral); #Guarda los BBH de otras familias que aparecen en el arbol
			}
		}
		# Ni agregar al outlabels
		## Id que no debe salir en arbol ##
		#
		}
	     elsif($giA[0]=~/CENTRAL/){
...
#-Estos son los BBH que salen con una unica familia--------------------------------------------------------------------- 
...
#-Estos son los BBH de otras familias que aparecen en el arbol----------------------------------------------------------
foreach my $cc (@QC2){
        print MESSAGES ("BBH ROSA");
        print EDIT "\n";
        if($cc =~/\./ or $cc eq '' or $cc eq ' '){
                #next;
                }
                print EDIT $NPc;
                print EDIT qq|"<circle style='fill:pink;stroke:black' r='6'/>" I $cc|; ## circle for central pathway in another family
                print EDIT "\n";
                }
